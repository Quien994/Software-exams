...
### 需求分析与系统设计  
>   在进行软件需求获取之前，首先要明确需要获取什么，也就是需求包含哪些内容。软件需求是指用户对目标软件系统存在功能、行为、性能、设计约束等方面的期望。通常这些需求包含功能需求、性能需求、用户或人的因素、环境因素、界面因素、文档需求、数据需求、资源使用需求、软件成本消耗与开发进度需求等，并预先估计以后系统可能达到的目标。  
...
#### 3. 详细设计  
  ...
  1. 对每个模块进行详细的算法设计  
  用某种图形、表格和语言等工具将每个模块处理过程的详细算法描述出来。  
  ...
  根据软件系统的类型，还可以进行以下设计  
    ...
    4.1 代码设计：为了提高数据的输入、分类、存储和检索等操作，节约内存空间，对数据库中某些数据项的值要进行代码设计  
    4.2 输入\输出格式设计  
    4.3 用户界面设计  
...
1. 逻辑覆盖  
  ...
  - 语句覆盖：语句覆盖是指选择足够的测试数据，使被测试程序中的每条语句至少执行一次  
  - 判定覆盖：判定覆盖是指设计足够的测试用例，使得被测程序中的每个判定表达式至少获得一次“真”值和“假”值
